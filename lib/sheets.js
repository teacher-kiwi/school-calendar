...
// Color hex map for FullCalendar rendering
const COLOR_HEX_MAP = {
...
const SheetsService = {
  ...
  async getEvents(categories = []) {
    try {
      ...
      rows.forEach((row) => {
        ...
        const colorHex = COLOR_HEX_MAP[catConfig?.color] || COLOR_HEX_MAP.gray;
        const backgroundColor = colorHex.bg;
        const borderColor = colorHex.border;
  ...
  /**
   * Get event owner email by ID
   */
  async getEventOwner(id) {
    const allRows = await getAllRows();
    const row = allRows.find((r) => r[0] === id);
    if (!row) return null;
    return row[8] || null; // col I: createdBy email
  },
